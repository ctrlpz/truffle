...
abstract class SteppingStrategy {
    ...
    private static final class StepEndTurn extends SteppingStrategy {
        ...
        boolean isActive(EventContext context, SuspendAnchor suspendAnchor) {
            ...
            //step to the next root node and check if the targetNode belongs to the suspended method/turn
        ...
        /**
         * Return the source section of the turn where this node belongs.
         * A turn is represented by the processing of an asynchronous message by an actor.
         * This method checks if the given node is located in a turn based on the selector
         * of the message that was sent.
         * Note: comparison using the source section is not possible at the moment.
         *
         * @param node
         * @return
         */
        private SourceSection getTurnSourceSectionForNode(Node node) {
            ...
            if (node instanceof RootNode) { //refers to the method
               ...
               if(node.getParent() instanceof WrapperNode) {
                  return getTurnSourceSectionForNode(node.getParent().getParent()); //refers to the child of the proxy node
               } else {
        ...
        private SourceSection getMethodSourceSectionForNode(Node node) {
            if (node instanceof RootNode) { //refers to the method
                ...
                if(node.getParent() instanceof WrapperNode) {
                    return getMethodSourceSectionForNode(node.getParent().getParent()); //refers to the child of the wrapper node
                } else {
